...
async function feedbackApi (feedback) {
  articles = {}; //await api.articles.postFeedback(id, language, feedback);
  return articles;
}
...
{/if}
<div>
    <button data-choice="yes" color="secondary" on:click="{() => window.myEvent('yes')}">{#if (typeof label !='undefined' && label.yes)}{label.yes}{:else}Yes{/if}</button> 
    <button option="outline" data-choice="no" color="secondary" on:click="{() => window.myEvent('no')}">{#if (typeof label !='undefined' && label.no)}{label.no}{:else}No{/if}</button>
</div>
{visible}
{#if visible}
<div>
    ...
    <div>
        ...
        <button option="flat" data-choice="submit" color="secondary" disabled={!feedback.Comment_if_No__c} on:click="{() => window.myEvent('submit')}">{#if (typeof label !='undefined' && label.commentSubmit)}{label.commentSubmit}{:else}Submit{/if}</button>					
    </div>
</div>
{/if}
{#if sucMsg}
