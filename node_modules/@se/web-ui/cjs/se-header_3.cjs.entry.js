...
const HeaderComponent = class {
    ...
    render() {
        ...
        if (this.domain.toLowerCase() === `ecostruxure`) {
            domain = index.h("se-icon-ecostruxure", { class: "header-title-type" });
        }
        else if (this.domain.toLowerCase() !== "none") {
            domain = index.h("span", { class: "header-title-type" }, this.domain);
        }
        return [
            index.h("div", { class: "d-flex" }, this.hasMenu ?
                index.h("span", { class: "menu-sidenav", onClick: () => this.onClickMenu() }, index.h("se-icon", { size: "medium", color: "primary" }, index.h("span", { innerHTML: burgerMenu })))
                : '', index.h("div", { class: "d-flex-column header-title-wrapper" }, domain, index.h("h1", { class: "header-title no-margin" }, index.h("span", null, title.first), index.h("span", { class: "light" }, "\u00A0", title.last))), this.project ? index.h("span", { class: "project-section" }, this.project) : ''),
            index.h("div", { class: "padding-container d-flex" }, index.h("slot", { name: "start" })),
            index.h("div", { class: "fill-space center-header-container" }, index.h("slot", null)),
            index.h("div", { class: "padding-container d-flex" }, index.h("slot", { name: "end" }), index.h("se-icon-schneider", { class: "header-title-type" }))
        ];
    }
...
const SidemenuComponent = class {
    ...
    renderList() {
        return this.items.map((item) => {
            return [
                index.h("se-list-item", { class: [!item.childElementCount ? 'hide-nav-icon' : '', 'sidemenu-list-item'].join(' '), option: "nav", onClick: () => this.setActive(item), selected: item.active, item: item.item, id: item.id ? `list-${item.id}` : '' }),
            ];
        });
    }
    componentWillLoad() {
    ...
    render() {
        return [
            index.h("div", { class: "menu-background animated d-flex-row flex", onClick: () => this.toggle(), ref: el => this.backdropEl = el }),
            index.h("div", { class: "actual-menu animated full-content d-flex-column flex", ref: el => this.menuInnerEl = el }, index.h("div", { class: "d-flex flex" }, index.h("se-block", { width: "250px" }, index.h("div", { class: "d-flex-center flex" }, index.h("span", { class: "menu-sidenav", onClick: () => this.toggle() }, index.h("se-icon", { size: "large", color: "primary" }, index.h("span", { innerHTML: test_results_nok }))), index.h("h3", { class: "header-title" }, this.label)), index.h("se-divider", null), index.h("se-block-content", { option: "fill" }, index.h("se-list", { option: "nav" }, this.renderList())), index.h("se-icon-lifeison", { class: "footer-icon", color: "standard" }), this.link ?
                index.h("div", { class: "external-link" }, index.h("se-link", { class: "sidemenu-link", url: `http://${this.link}` }, this.link)) : ''), index.h("se-divider", { option: "vertical" }), index.h("se-block", { ref: el => this.menuItemInnerEl = el }, index.h("slot", null))))
        ];
    }
    get el() { return index.getElement(this); }
};
...
const SidemenuItemComponent = class {
    ...
    render() {
        return (index.h(index.Host, { role: 'sidenavpanel', "aria-hidden": { 'true': !this.active }, class: [this.active ? 'active' : 'navitem-hidden'].join(' ') }, index.h("slot", null)));
    }
...
exports.se_header = HeaderComponent;
exports.se_sidemenu = SidemenuComponent;
exports.se_sidemenu_item = SidemenuItemComponent;
