...
const listCss = ":host{width:100%;height:100%;position:relative;overflow:auto;-moz-flex-direction:column;-ms-flex-direction:column;flex-direction:column}";
...
const ListGroupComponent = class {
    constructor(hostRef) {
         ...
         */
        this.canCollapse = true;
        this.didGroupClick = index.createEvent(this, "didGroupClick", 7);
        this.didGroupCollapse = index.createEvent(this, "didGroupCollapse", 7);
    }
    collapsedChanged() {
        this.checkSelected();
    }
    ...
    render() {
        ...
        if (!!this.description) {
            myDescription = index.h("small", null, this.description);
        ...
        return (index.h(index.Host, { role: "listitem", option: this.option }, index.h("button", { "aria-expanded": `${this.collapsed}`, title: title, id: this.innerId, class: ['se-list-group', this.option, this.collapsed ? "collapsed" : '', this.option, this.selected ? "selected" : '', this.selectedChild ? "selectedChild" : '', "button"].join(' '), style: { paddingLeft: `${20 * this.indentation}px` }, onClick: () => this.toggleGroupButton(), disabled: !this.canCollapse }, this.option === "nav" && this.selected && index.h("div", { class: "selectedBar" }), this.option === 'treeview' ? index.h("se-icon", { class: "treeview-collapse-icon", onClick: (e) => { e.stopPropagation(); this.toggleCollapseTreeview(); }, style: { paddingLeft: `calc(${8 * this.indentation}px)` } }, index.h("span", { innerHTML: this.collapsed ? arrow2_right.arrow2_right : arrow2Down })) : '', this.hasIcon &&
            index.h("div", { class: "nav-icon" }, this.icon && index.h("se-icon", { color: this.iconColor }, this.icon), index.h("slot", { name: "icon" })), index.h("div", { class: "nav-content" }, index.h("div", { class: "list-group-label" }, this.item), myDescription), this.option !== "treeview" && this.canCollapse
            ? index.h("se-icon", { class: "standard-collapse-icon", size: "medium" }, index.h("span", { innerHTML: this.collapsed ? arrow2Down : arrow2Up }))
            : ''), index.h("div", { role: "list", class: ["se-list-group", "group-item", this.option].join(' ') }, index.h("slot", null))));
    }
    get el() { return index.getElement(this); }
    static get watchers() { return {
...
const ListItemComponent = class {
    constructor(hostRef) {
         ...
         */
        this.indentation = 0;
        this.didSelectedChange = index.createEvent(this, "didSelectedChange", 7);
    }
    SelectedDidChange() {
    ...
    render() {
        ...
        const icon = this.option !== "treeview" ?
            index.h("se-icon", { color: this.iconColor }, this.icon)
            :
                this.indentation !== 0 ?
                    index.h("se-icon", { color: this.iconColor, style: { paddingLeft: `8px` } }, this.icon)
                    :
                        index.h("se-icon", { color: this.iconColor }, this.icon);
        const padding = this.option !== "treeview" ? 20 : 24;
        return (index.h(index.Host, { role: "listitem" }, index.h(TagType, Object.assign({}, attrs, { title: title, class: { "selected": this.selected, ["button"]: true, [this.option]: true, ["se-list-item"]: true }, style: { paddingLeft: `${padding * this.indentation}px` }, id: this.innerId }), (this.option === "nav" && this.selected) ? index.h("div", { class: "selectedBar" }) : '', index.h("div", { class: "nav-icon" }, !!this.icon ? icon : '', index.h("slot", { name: "icon" })), index.h("div", { class: "nav-content" }, index.h("div", { class: "list-item-label" }, this.item), myDescription), index.h("slot", null), this.option === "nav" ? index.h("se-icon", { size: "medium" }, index.h("span", { innerHTML: arrow2_right.arrow2_right })) : '')));
    }
    get el() { return index.getElement(this); }
    static get watchers() { return {
...
exports.se_list = ListComponent;
exports.se_list_group = ListGroupComponent;
exports.se_list_item = ListItemComponent;
